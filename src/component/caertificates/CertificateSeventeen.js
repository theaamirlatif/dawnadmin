...
const CertificateSeventeen = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EG/AB-9882")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Analytical Balance")
    const [section, setSection] = useState("Q.C. LAB.")
    const [model, setModel] = useState("KD-1004K")
    const [labTemp, setLabTemp] = useState("25°C")
    const [instrId, setInstrId] = useState("EG/QC/EQP/003")
    const [labHumidity, setlabHumidity] = useState("50% RH")
    const [manufacturer, setManufacturer] = useState("SPECTRUM SCIENTIFIC")
    ...
    const [unit1, setUnit1] = useState("gm")
    const [traceMeasure, setTraceMeasure] = useState("The Analytical Balance has been calibrated against Pre-calibrated set of dead weight, Model/Make: China/M1, that is traceable to the certificate number APCIC/MF (260); ILO # 425/10.06.21, PCSIR which in turns traceable to Procedure # LLC/APCIC/MCP/01, Cal.Lab-M24, Report # CC-10687 NPSL Pakistan.")
    const [abParameters, setAbParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }])
    const [ab1Parameters, setAb1Parameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }])
    ...
    useEffect(async () => {
        ...
        setAbParameters(cusRes.parameter1)
        setAb1Parameters(cusRes.parameter2)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...abParameters]
        list[index][name] = value
        setAbParameters(list)
    }
    ...
    const handleAddClick = () => {
        setAbParameters([...abParameters, { appliedValue: "", observedValue: "", deviation: "" }])
    }
    ...
    const handleInputChange1 = (e, index) => {
        const { name, value } = e.target
        const list = [...ab1Parameters]
        list[index][name] = value
        setAb1Parameters(list)
    }
    ...
    const handleAddClick1 = () => {
        setAb1Parameters([...ab1Parameters, { appliedValue: "", observedValue: "", deviation: "" }])
    }
    ...
    async function Submit() {
        ...
        formData.append('ab_Parameters', JSON.stringify(abParameters))
        formData.append('ab1_Parameters', JSON.stringify(ab1Parameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addSeventeenCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {abParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {abParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
                        ...
                        return (
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {ab1Parameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick1(i)}></i>}
                                        {ab1Parameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick1}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateSeventeen
