...
const CertificateThirtyThree = () => {
    ...
    const [cCertificate, setCCertificate] = useState("VL /RM-9005")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Refrectometer")
    const [section, setSection] = useState("Q.C Lab.")
    const [model, setModel] = useState("IR120")
    const [labTemp, setLabTemp] = useState("25°C")
    const [instrId, setInstrId] = useState("SOP/QC/INS/WI/010")
    const [labHumidity, setlabHumidity] = useState("50% RH")
    const [manufacturer, setManufacturer] = useState("Insmark")
    const [calibDate, setCalibDate] = useState(date)
    const [client, setClient] = useState("")
    const [nextCalib, setNextCalib] = useState(todate)
    const [issueDate, setIssueDate] = useState(date)
    const [unit, setUnit] = useState("Nil")

    const [traceMeasure, setTraceMeasure] = useState("The Refrectometer has been calibrated against, USP reference standard water, Sample I, sample II, sample III (SUPELCO) and sorbitol, Certificate #124-45-8 which are traceable to NIST (USA).")

    const [rmParameters, setRmParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }]);
    ...
    useEffect(async () => {
        ...
        setRmParameters(cusRes.parameter1)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...rmParameters];
        list[index][name] = value;
        setRmParameters(list);
    };
    ...
    const handleAddClick = () => {
        setRmParameters([...rmParameters, { appliedValue: "", observedValue: "", deviation: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('rm_Parameters', JSON.stringify(rmParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addThirtyThreeCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {rmParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {rmParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateThirtyThree