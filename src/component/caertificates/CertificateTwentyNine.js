...
const CertificateTwentyNine = () => {
    ...
    const [cCertificate, setCCertificate] = useState("SP /FTA-0494")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Friability Tester")
    const [section, setSection] = useState("QC. Lab.")
    const [model, setModel] = useState("QEHS")
    const [labTemp, setLabTemp] = useState("25°C")
    const [instrId, setInstrId] = useState("QC/INS/027")
    ...
    useEffect(async () => {
        ...
        setFtaParameters(cusRes.parameter1)
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...ftaParameters];
        list[index][name] = value;
        setFtaParameters(list);
    };
    ...
    const handleAddClick = () => {
        setFtaParameters([...ftaParameters, { appliedValue: "", observedValue: "", deviation: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('fta_Parameters', JSON.stringify(ftaParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addTwentyNineCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {ftaParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {ftaParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateTwentyNine