...
const CertificateTwenty = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EP /AM-0859")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Anemometer")
    const [section, setSection] = useState("Micro Lab.")
    const [model, setModel] = useState("AM-4836V")
    ...
    const [traceMeasure, setTraceMeasure] = useState("The anemometer has been calibrated at 25○C against the pre-calibrated Digital temperature controller, SWIYS, Model # SW-72BD, Certificate # S & M / TCF / 06 (796), Which is traceable to International Bureau of weights and measures, France And Asia Pacific Metrology Program (APMP) through NPSL and for anemometer Certificate # 12-234-AM which is traceable to NIST.")

    const [amParameters, setAmParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }])
    ...
    useEffect(async () => {
        ...
        setAmParameters(cusRes.parameter1)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...amParameters]
        list[index][name] = value
        setAmParameters(list)
    }
    ...
    const handleAddClick = () => {
        setAmParameters([...amParameters, { appliedValue: "", observedValue: "", deviation: "" }])
    }
    ...
    async function Submit() {
        ...
        formData.append('am_Parameters', JSON.stringify(amParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addTwentyCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {amParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {amParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        );
...
export default CertificateTwenty