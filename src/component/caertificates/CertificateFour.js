...
const CertificateFour = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EG/HPLC-P-9878")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("HPLC Pump")
    ...
    useEffect(async () => {
        ...
        setFlowRate(cusRes.parameter1)
    }, [])

    

    // handle input change
    const handleInputChangeF = (e, index) => {
    ...
    const handleRemoveClickF = index => {
        const list = [...flowRate];
        list.splice(index, 1);
        setFlowRate(list);
    };
    ...
    const handleAddClickF = () => {
        setFlowRate([...flowRate, { appliedValue: "", observedValue: "", deviation: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('flowRate', JSON.stringify(flowRate))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addFourCertificate", {
            method: 'POST',
            body: formData
        });
                                ...
                                <div className="row">
                                    <div className="col-sm-4 p-0">
                                        <div style={{ marginLeft: '10px' }}>
                                            <input type="text" name="appliedValue" className='form-control form-control-sm' value={x.appliedValue} onChange={e => handleInputChangeF(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-4 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="observedValue" className='form-control form-control-sm' value={x.observedValue} onChange={e => handleInputChangeF(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-3 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="deviation" className='form-control form-control-sm' value={x.deviation} onChange={e => handleInputChangeF(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {flowRate.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClickF(i)}></i>}
                                        {flowRate.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClickF}></i>}
                                    </div>
                                </div>
                            </>
                        );
...
export default CertificateFour
