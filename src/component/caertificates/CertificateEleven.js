...
const CertificateEleven = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EG/TC-1027")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Temperature Controller (Bottle Filling Machine)")
    const [section, setSection] = useState("Oral Liquid Filling")
    const [model, setModel] = useState("REX-C100")
    const [labTemp, setLabTemp] = useState("25°C")
    const [instrId, setInstrId] = useState("EG/PRD/LS/18")
    const [labHumidity, setlabHumidity] = useState("50% RH")
    const [manufacturer, setManufacturer] = useState("FRIENDS")
    ...
    useEffect(async () => {
        ...
        setTcParameters(cusRes.parameter1)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...tcParameters];
        list[index][name] = value;
        setTcParameters(list);
    };
    ...
    const handleAddClick = () => {
        setTcParameters([...tcParameters, { appliedValue: "", observedValue: "", deviation: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('tc_Parameters', JSON.stringify(tcParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addElevenCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {tcParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {tcParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        );
                    })}
                    {/* <div style={{ marginTop: 20 }}>{JSON.stringify(tcParameters)}</div> */}
                    <hr />
                    <div className='row'>
                        <div className='col-sm-12'>
                            <label><b>Traceability of Measurements:</b></label>
...
export default CertificateEleven
