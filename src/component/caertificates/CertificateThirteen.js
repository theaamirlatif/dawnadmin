...
const CertificateThirteen = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EG/FTIR - 9881")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("FTIR Spectrophotometer")
    const [section, setSection] = useState("QC. Lab.(Instrument Room)")
    const [model, setModel] = useState("FTIR-7600")
    const [labTemp, setLabTemp] = useState("25°C")
    const [instrId, setInstrId] = useState("EG/QC/EQP/32")
    const [labHumidity, setlabHumidity] = useState("50% RH")
    const [manufacturer, setManufacturer] = useState("LAMBDA")
    ...
    const [observation, setObservation] = useState("1. The Values on the Spectra of  (Poly styrene) and Peak pattern complies with the polystyrene reference standard spectra. 2:	Spectra of Polystyrene is attached.")
    const [traceMeasure, setTraceMeasure] = useState("The FTIR Spectrophotometer has been calibrated at laboratory 25 ℃ and Humidity 51% against Polystyrene reference material which is traceable to CC#16322, Lambda Scientific Australia ")
    ...
    // const [ftParameters, setFtParameters] = useState([{ ref: "", appliedValue: "", observedValue: "", deviation: "" }]);

    // // handle input change
    // const handleInputChange = (e, index) => {
    //     const { name, value } = e.target;
    //     const list = [...ftParameters];
    //     list[index][name] = value;
    //     setFtParameters(list);
    // };
    ...
    //     setFtParameters([...ftParameters, { ref: "", appliedValue: "", observedValue: "", deviation: "" }]);
    // };
    ...
    async function Submit() {
        ...
        // formData.append('ft_Parameters', JSON.stringify(ftParameters))
        formData.append('observation', observation)
        formData.append('trace_measure', traceMeasure)
        let result = await fetch(window.api + "addThirteenCertificate", {
            method: 'POST',
            body: formData
        })
        ...
        <>
            <div className="card card-primary">
                ...
                <div className="card-body">
                    ...
                    {ftParameters.map((t, i) => {
                        return (
                            <>
                                <div className="row">
                                    <div className="col-sm-2 p-0">
                                        <div style={{ marginLeft: '10px' }}>
                                            <input type="text" name="ref" className='form-control form-control-sm' value={t.ref} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {ftParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {ftParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        );
                    ...
                    <div className='row'>
                        <div className='col-sm-12'>
                            <lable><b> Observation:</b></lable>
                            <textarea className='form-control' value={observation} onChange={(e) => setObservation(e.target.value)}>
                            </textarea>
                        </div>
                    </div>
                    <hr />
...
export default CertificateThirteen
