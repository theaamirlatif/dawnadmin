...
const CertificateNineteen = () => {
    ...
    const [cCertificate, setCCertificate] = useState("RP/PC-9903")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Particle Counter")
    ...
    const [manufacturer, setManufacturer] = useState("CEM")
    const [calibDate, setCalibDate] = useState(date)
    const [client, setClient] = useState("")
    const [nextCalib, setNextCalib] = useState(todate)
    const [issueDate, setIssueDate] = useState(date)
    const [traceMeasure, setTraceMeasure] = useState("The particle counter has been calibrated at 25○C against pre-calibrated particle counter model # CLJ-H603 that is traceable to certificate # 051012-PC, Lab # 60, Spectrum Scientific-UK (UKAS).")
    const [unit, setUnit] = useState("micron")
    const [pcParameters, setPcParameters] = useState([{ partSize: "", restCondition: "", operCondition: "", deviation: "" }])
    ...
    useEffect(async () => {
        ...
        setPcParameters(cusRes.parameter1)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...pcParameters]
        list[index][name] = value
        setPcParameters(list)
    }
    ...
    const handleAddClick = () => {
        setPcParameters([...pcParameters, { partSize: "", restCondition: "", operCondition: "", deviation: "" }])
    }
    ...
    async function Submit() {
        ...
        formData.append('pc_parameters', JSON.stringify(pcParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addNineteenCertificate", {
            method: 'POST',
            body: formData
        });
        ...
        <>
            <div className="card card-primary">
                ...
                <div className="card-body">
                    ...
                    <hr />
                    <div className='row'>
                        <div className='col-sm-1'><label>Manufacturer: </label></div>
                        <div className='col-sm-3'>
                    ...
                    {pcParameters.map((p, i) => {
                        return (
                            <>
                                <div className="row">
                                    <div className="col-sm-3 p-0">
                                        <div style={{ marginLeft: '10px' }}>
                                            <input type="text" name="partSize" className='form-control form-control-sm' value={p.partSize} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-3 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="restCondition" className='form-control form-control-sm' value={p.restCondition} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-3 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="operCondition" className='form-control form-control-sm' value={p.operCondition} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {pcParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {pcParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateNineteen;
