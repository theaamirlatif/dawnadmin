...
const CertificateTen = () => {
    ...
    const [cCertificate, setCCertificate] = useState("EG/T - 1020")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Timer (Bottle Filling Machine)")
    const [section, setSection] = useState("Oral Liquid Filling")
    const [model, setModel] = useState("DH48S")
    ...
    const [traceMeasure, setTraceMeasure] = useState("The Stop Watch has been calibrated against pre-calibrated stop watch , Model/Make: CATIGA/ CG 502, that is traceable to the certificate number APCIC/EL (822); ILO # 391/12.11.2021, PCSIR which in turns traceable to Procedure # LLC/APCIC/WCP/01 LLC, Cal. Lab- t2 & t3, Report # 11035 & 11161 NPSL Pakistan.")

    const [tParameters, setTParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "", uncertainty: "" }]);
    ...
    useEffect(async () => {
        ...
        setTParameters(cusRes.parameter1)
    }, [])
    ...
    const handleAddClick = () => {
        setTParameters([...tParameters, { appliedValue: "", observedValue: "", deviation: "", uncertainty: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('t_Parameters', JSON.stringify(tParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addTenCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-2 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="uncertainty" className='form-control form-control-sm' value={t.uncertainty} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {tParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {tParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        );
