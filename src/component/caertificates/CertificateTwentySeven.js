...
const CertificateTwentySeven = () => {
    ...
    const [cCertificate, setCCertificate] = useState("MRL/DWB-7864")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Dead Weight Box")
    const [section, setSection] = useState("Dispensing Room")
    ...
    const [traceMeasure, setTraceMeasure] = useState("The dead weight has been calibrated against Pre-calibrated set of mass, Model/Make: China/MI, that is traceable to the certificate number APCIC/MF(202); ILO # 324/13.05.2020, LLC, PCSIR which in turns traceable to Report # CC-8434, 8435, NPSL Pakistan.")

    const [dwbmParameters, setDwbmParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }]);
    ...
    useEffect(async () => {
        ...
        setDwbmParameters(cusRes.parameter1)
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...dwbmParameters];
        list[index][name] = value;
        setDwbmParameters(list);
    };
    ...
    const handleAddClick = () => {
        setDwbmParameters([...dwbmParameters, { appliedValue: "", observedValue: "", deviation: "" }]);
    };
    ...
    async function Submit() {
        ...
        formData.append('dwbm_Parameters', JSON.stringify(dwbmParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addTwentySevenCertificate", {
            method: 'POST',
            body: formData
        });
                            ...
                            <>
                                <div className="row">
                                    ...
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {dwbmParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {dwbmParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateTwentySeven