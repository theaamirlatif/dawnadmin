...
const CertificateTwentyFive = () => {
    ...
    const [cCertificate, setCCertificate] = useState("VL /CM-9010")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Conductivity Meter")
    const [section, setSection] = useState("QC. Lab.")
    const [model, setModel] = useState("DDS-307")
    const [labTemp, setLabTemp] = useState("25°C")
    const [tempLabel, setTempLabel] = useState("")
    const [instrId, setInstrId] = useState("VL/QC/2/04")
    ...
    const [unit, setUnit] = useState("µs/cm")
    const [traceMeasure, setTraceMeasure] = useState("The conductivity meter is calibrated at 25○C against certified reference standard CAS # 7447-40-7, Cat # 1548190 and Batch No = FODO42, which in turn traceable to NIST (USA).")
    ...
    async function Submit() {
        ...
        formData.append('cm1_Parameters', JSON.stringify(cmParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api + "addTwentyFiveCertificate", {
            method: 'POST',
            body: formData
        });
...
export default CertificateTwentyFive