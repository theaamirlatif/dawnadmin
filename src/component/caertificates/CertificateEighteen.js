...
const CertificateEighteen = () => {
    ...
    const [cCertificate, setCCertificate] = useState("RP/KF-9602")
    const [dawnDataSheet, setDawnDataSheet] = useState("")
    const [equipment, setEquipment] = useState("Karl Fischer")
    const [section, setSection] = useState("Q.C. Lab.")
    const [model, setModel] = useState("22 KF")
    ...
    const [unit, setUnit] = useState("% Water Contents")
    const [traceMeasure, setTraceMeasure] = useState("The Karl Fischer has been calibrated at 25○C against five-reference standards, which is traceable to the certificate # 02178-5 that is traceable to the NIST (USA).")
    const [kfParameters, setKfParameters] = useState([{ appliedValue: "", observedValue: "", deviation: "" }])
    ...
    useEffect(async () => {
        ...
        setKfParameters(cusRes.parameter1)
    }, [])
    ...
    const handleInputChange = (e, index) => {
        ...
        const list = [...kfParameters]
        list[index][name] = value
        setKfParameters(list)
    }
    ...
    const handleAddClick = () => {
        setKfParameters([...kfParameters, { appliedValue: "", observedValue: "", deviation: "" }])
    }
    ...
    async function Submit() {
        ...
        formData.append('kf_parameters', JSON.stringify(kfParameters))
        formData.append('trace_measure', traceMeasure)

        let result = await fetch(window.api+"addEighteenCertificate", {
            method: 'POST',
            body: formData
        });
        ...
        <>
            <div className="card card-primary">
                ...
                <div className="card-body">
                    ...
                    {kfParameters.map((k, i) => {
                        return (
                            <>
                                <div className="row">
                                    <div className="col-sm-4 p-0">
                                        <div style={{ marginLeft: '10px' }}>
                                            <input type="text" name="appliedValue" className='form-control form-control-sm' value={k.appliedValue} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-4 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="observedValue" className='form-control form-control-sm' value={k.observedValue} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-3 p-0">
                                        <div style={{ marginLeft: '2px' }}>
                                            <input type="text" name="deviation" className='form-control form-control-sm' value={k.deviation} onChange={e => handleInputChange(e, i)} />
                                        </div>
                                    </div>
                                    <div className="col-sm-1" style={{ paddingTop: '8px' }}>
                                        {kfParameters.length !== 1 && <i className='fas fa-times-circle' onClick={() => handleRemoveClick(i)}></i>}
                                        {kfParameters.length - 1 === i && <i className='fas fa-plus-circle' onClick={handleAddClick}></i>}
                                    </div>
                                </div>
                            </>
                        )
...
export default CertificateEighteen
